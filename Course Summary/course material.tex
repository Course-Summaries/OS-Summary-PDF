...
\begin{center}
  ...
  \begin{minipage}{0.5\textwidth}
    ...
    \hspace*{2em}However, each thread has its own stack, \ul{kernel stack}, registers, and program counter. But they can still access each other's stack since they share the same memory space.
  \end{minipage}
\end{center}
...
\paragraph{Spinlock vs. Mutex Lock} We define each one as follows:
\begin{itemize}
  \item \textbf{\red{Spinlock}:} When a thread tries to catch the lock, it will keep trying again and again \red{without leaving the run queue}. It can preempt, but not leave the run queue. 
  \item \textbf{\red{Mutexlock}:} When a thread tries to catch the lock, the \ul{operating system} will move it to the \red{wait queue} and do a context switch.
\end{itemize}
