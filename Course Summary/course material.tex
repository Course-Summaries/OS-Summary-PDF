...
\textbf{Note:} The Banker's Algorithm is \blue{conservative}, i.e. it will not allocate resources even if there is possibiliy to allocate them without deadlocking the system. The OS doesn't take chances! With \blue{Time Complexity \(O(n^2)\)}.

\begin{lstlisting}[language=C, caption={Banker's Algorithm}, label={lst:bankers_algorithm}]
// *Asume* that request R was granted to process q
